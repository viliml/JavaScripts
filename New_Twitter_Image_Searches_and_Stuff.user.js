...
async function GetImageURL(image_info) {
...
function GetImageURLInfo(image_url) {
    for (let i = 0; i < HANDLED_IMAGES2.length; i++) {
        let match = XRegExp.exec(image_url, HANDLED_IMAGES2[i]);
        if (match) {
            return match.groups;
        }
    }
...
async function GetImageLinks(tweet) {
    if (!tweet.ntisasImageUrls) {
        ...
        for (let i = 0; i < $obj.length; i++) {
            ...
            let image_url = await GetImageURL(image_info);
            if (image_url) {
...
function GetEventPreload(event, classname) {
    ...
    let $replace = $(`[data-tweet-id=${tweet_id}] .${classname}`);
